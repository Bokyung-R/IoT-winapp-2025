using System.ComponentModel;    // 백그라운드 워커 이벤트아큐먼트 네임스페이스
...
namespace SyntaxWinApp01
{
    public partial class FrmMain : Form
    {
        ...
        {
            // 백그라운드 워커 초기화
            BgwProcess.WorkerReportsProgress = true;    // 진행사항 리포트 할 것 인지
        ...
        private void BgwProcess_DoWork(object? sender, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i < MaxVal; i++)
            {
                ...
                // 진행사항을 ProgressChanged 이벤트 쪽으로 전달(넘김)
                BgwProcess.ReportProgress(progress, i);

                Thread.Sleep(50);   // 50ms 시간 소요 (무거운 작업 흉내)
            }
        }
        ...
        {
            int progress = e.ProgressPercentage;        // Dowork.ReportProgress(1,2) 중 1번으로 넘어 온 값
            ...
            TxtLog.ScrollToCaret();     // 텍스트 박스 제일 마지막 줄로 이동

            LblState.Text = $"현재 상태 : {progress}%";
        }
